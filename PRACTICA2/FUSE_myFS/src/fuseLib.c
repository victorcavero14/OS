...
//Exercise 4: sale de sumar 3*4096 + 5*4096 (SUPERBLOCK, BITMAP, DIRECTORY y INODES)
//	hexdump virtual-disk -C -s 61440 -n 15
...
static int my_unlink(const char* path)
{
	...
	dir = findFileByName(&myFileSystem,(char*)path+1); //Busqueda de fichero
    
	nod = myFileSystem.directory.files[dir].nodeIdx; //Busqueda de nodo
    
	resizeNode(nod,0); //Redimensionamos el nodo y el fichero

	myFileSystem.directory.files[dir].freeFile = true;
	myFileSystem.directory.numFiles--;
	myFileSystem.numFreeNodes++;
	myFileSystem.nodes[nod]->freeNode = true;

	updateDirectory(&myFileSystem);
	updateNode(&myFileSystem,nod,myFileSystem.nodes[nod]);
	free(myFileSystem.nodes[nod]);
	myFileSystem.nodes[nod] = NULL;
...
{
	char buffer[BLOCK_SIZE_BYTES];
	int currentBlock, offBlock, i, dir, bytes2Read = size, totalRead = 0;
	NodeStruct *node = myFileSystem.nodes[fi->fh];

	fprintf(stderr, "--->>>my_read: path %s, size %zu\n", path, size);

	if((dir = findFileByName(&myFileSystem, (char *)path + 1)) == -1) //Error si no encuentra el fichero
	{
		return -ENOENT;
	}

    while(bytes2Read) //Bucle hasta que se acaben los bytes*bloque del fichero leído
    {
        currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
        offBlock = offset % BLOCK_SIZE_BYTES;

        if(readBlock(&myFileSystem, currentBlock, &buffer)==-1) //Lectura de bloque
        {
