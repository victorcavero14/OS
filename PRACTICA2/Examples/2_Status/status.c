/*
 ...
 Para poder añadir una opcion nueva creamos un while que trate de obtener todas
 las que esten seguidas de un "-".
 Ademas en la funcion status le añadimos un optionL para saber cuando esta es la escogida
 y fue necesario modificar en la misma la funcion stat (que sigue el enlace simbolico por
 defecto) y utilizar la funcion lstat.
 *
 */
...
int status(char *filename,int optionL) {
	...
	switch(buf.st_mode & S_IFMT) {
        ...
        case S_IFLNK:
            ...
            //printf(readlink(filename,stdout,sizeof(stdout)));
            break;
        default:
            printf("UNKNOWN.\n"); break;
	}
